;
((c) => {
    //1. Asignación implicita:
    //Caso 1
    //this está siendo invocado dentro de un método
    //this, hace referencia al objeto, que tiene el método que se invoca
    c('**********Asígnación de this Implicita**********');

    let yo = {
        nombre: 'Gerardo Agustin',
        edad: 46,
        saludar: function() {
    ...
    preparandoSaludo(gera);
    preparandoSaludo(ale);
    ...
    // Caso 3
    // Una función que retorna un objeto, que contiene un método que que invoca this
    let Humano = function(nombre, edad, perro) {
        return {
            ...
            perro: {
                nombre: perro,
                saludar: function() {
                    c(`${this.nombre} guauuuu guauuuuu`)
                }
            }
        };
    };

    const persona = Humano('Gerardo', 46, 'yoshi');
    persona.saludar();
    persona.perro.saludar();
...
((c) => {
    ...
    c('**********Asignación de this Explícitamente**********');

    const nombrar = function(f1, f2, f3) {
        c(`${this.nombre} es el lenguaje FrontEnd de la web y tiene librerías y frameworks muy poderosos como: ${f1}, ${f2}, ${f3}`);
    };
    ...
    //apply: Permite referenciar this en el primer parámetro, pero este nos permite pasar un array, como los parámetros de la función.
...
((c) => {
    ...
    //Cuando invocamos this en un constructor, este hace referencia al objeto que se ha instanciado
