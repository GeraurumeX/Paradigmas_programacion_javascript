((c) => {
    ...
    class Carrito {
        ...
        ver(articulo = 'todos') {
            return (articulo === 'todos') ? this._iterable() :
                (this._carrito.hasOwnProperty(articulo)) ? `${this._carrito[articulo]} ${articulo}` :
...
((c) => {
    ...
    c('**********Herencia, Polimorfismo, Método Constructor, Setter y Getters, Módificadores de Acceso (público, estático, privado), Super Constructores, Super Llamadas y Mixins**********');
    ...
    class Telefono {
        ...
        constructor(marca, modelo, numero) {
            ...
            this.modelo = modelo;
            //this._numero = numero;
            //Cuando es privado se usa lo siguiente:
            privado.set(this, { _numero: numero });
            this.puedoLlamar = true;
        }

        //un método estático se puede ejecutar sin necesidad de instanciar la clase, por lo general, este tipo de métodos se reservan a clases que coleccionan utilidades y que no se espera que sean instanciadas.
        //Estamos hablando de los típicos 'helpers' habituales de la mayoría de frameworks y librerias
        static queEs() {
    ...
    class Celular extends Operadora(Red(Telefono)) {
        ...
        vibrar() {
                c('Vbbbbbbbrrrrrr   Vbbbbbbbbbrrrrrr!!!!');
            }
            //Polimorfismo: Diferentes Clases podrían definir el mismo método o propiedad
        verInfo() {
            //Superllamada: con super se llama el método verInfo() de la clase padre
    ...
    let sm = new Smartphone('Motorola', 'G4', '044550099887766');
    sm.verInfo();
    sm.llamar();
    sm.vibrar();
    sm.numero;
    sm.numero = '044551122334455';
    sm.numero;
